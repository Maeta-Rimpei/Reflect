...
export function SignInPanel() {
  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. 検証成功 → NextAuth でセッション発行（リダイレクト）
          ...
          <form onSubmit={handleEmailSignIn} className="space-y-3">
            ...
            <div className="relative">
              <input
                ...
                placeholder="パスワード"
                required
                className="w-full rounded-lg border border-border bg-background px-3 py-2.5 pr-10 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-1 focus:ring-foreground/20"
              />
              <button
                type="button"
